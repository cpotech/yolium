...
export function FavoritesList({
  ...
  return (
    <div className="mt-2">
      <div className="text-xs text-[var(--color-text-muted)] uppercase tracking-wide mb-1 flex items-center gap-1">
        <svg className="w-3 h-3 text-yellow-500" fill="currentColor" viewBox="0 0 20 20">
          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
        </svg>
        Favorites
      </div>
      <ul className="border border-[var(--color-border-primary)] rounded-md bg-[var(--color-bg-primary)] max-h-32 overflow-y-auto">
        {favorites.map((favPath, index) => {
          ...
          return (
            <li
              ...
              className="px-3 py-1.5 cursor-pointer flex items-center gap-2 text-[var(--color-text-primary)] hover:bg-[var(--color-bg-tertiary)] group"
            >
              {shortcutNum && (
                <span className="text-xs text-[var(--color-text-disabled)] font-mono w-4" title={`Ctrl+${shortcutNum}`}>
                  {shortcutNum}
                </span>
              )}
              <svg
              ...
              <button
                ...
                className="p-0.5 hover:bg-[var(--color-bg-hover)] rounded opacity-0 group-hover:opacity-100 transition-opacity"
